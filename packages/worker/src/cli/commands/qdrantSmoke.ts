import { getConfig, createLogger, createQdrantClient, ensureCollection } from "@kol-hatorah/core";
...
export async function qdrantSmokeTest() {
  ...
  try {
    const collectionName = `${config.qdrant.collectionPrefix}_smoke_test`;
    ...
    await ensureCollection(client, collectionName, 768);
